...
def create_app():
    ...
    app.config["SECRET_KEY"] = app.config.get("SECRET_KEY", "tetris-secret")
    ...
    def handle_create_room(data=None):
        ...
        emit("player_joined", {"room": room.to_dict(), "player": room.get_player(request.sid).to_dict()}, room=room.room_id)
    ...
    def handle_join_room(data):
        ...
        if error:
            ...
            return
        join_room(room.room_id)
        emit("player_joined", {"room": room.to_dict(), "player": room.get_player(request.sid).to_dict()}, room=room.room_id, include_self=True)
    ...
    def handle_game_state_update(data):
        ...
        emit("opponent_state", {"sid": request.sid, "grid": data.get("grid"), "score": player.score, "lines_cleared": player.lines_cleared, "game_over": data.get("game_over", False)}, room=room.room_id, include_self=False)
    ...
    def handle_lines_cleared(data):
        ...
        for p in room.players:
            if p.sid != request.sid and not p.game_over:
                emit("garbage_lines", {"lines": garbage, "from_sid": request.sid}, to=p.sid)
